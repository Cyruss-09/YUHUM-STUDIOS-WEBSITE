  ...
  return (
    <>
      ...
      >
              ...
              <div className="relative hidden md:block" ref={profileRef}>
                ...
                >
                  <div className="px-4 py-2.5 border-b border-[#E8DFD1]">
                    <p className="text-sm font-semibold text-[#2C221E] truncate">{user.username}</p>
                    <p className="text-xs text-[#7A6B63] truncate">{user.email}</p>
                  </div>
                  <button
        ...
        {/* Mobile menu — a simple dropdown panel, not a full-screen takeover */}
        <div
        ...
        >
          <nav className="max-w-7xl mx-auto pt-4 pb-2">
            <ul className="flex flex-col list-none m-0 p-0">
              ...
              <li className="pt-4 mt-2 border-t border-[#E8DFD1]">
                {user ? (
                  <div className="flex items-center justify-between py-2">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-[#A3704C] text-white text-sm font-semibold flex items-center justify-center">
                        {getInitials(user.username)}
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-[#2C221E]">{user.username}</p>
                        <p className="text-xs text-[#7A6B63]">{user.email}</p>
                      </div>
                    </div>
                    <button
                      onClick={handleLogout}
                      className="text-sm font-medium text-[#A3704C] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#A3704C] rounded-sm"
                    >
                      Log out
                    </button>
                  </div>
                  ...
                  <a
                    href={loginItem.path}
                    onClick={(e) => handleLinkClick(e, loginItem.id, loginItem.path)}
                    className="block text-center rounded-full bg-[#A3704C] py-3 text-sm font-medium text-[#FBF9F5] hover:bg-[#8C5A35] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#A3704C] focus-visible:ring-offset-2 focus-visible:ring-offset-[#FBF9F5]"
                  >
                    {loginItem.label}
                  </a>
                )}
