...
// Core increment logic, reusable from anywhere in the backend (e.g. bookingController)
// without going through HTTP. Returns { success, message? , data? } instead of writing
// to a `res` object, since it's meant to be called as a plain function too.
const incrementPromoUsage = async (code) => {
    ...
    if (!updated) {
        ...
        return { success: false, message: 'This promo code was just used up. Please try again.' };
    }

    return { success: true, data: updated };
};

// HTTP route wrapper around incrementPromoUsage — call this ONLY when a booking/order
// is actually confirmed, not on every validation check.
const applyPromoCode = async (req, res) => {
    const { code } = req.params;

    try {
        const result = await incrementPromoUsage(code);

        if (!result.success) {
            const status = result.message.includes('used up') ? 409
                : result.message.includes('Invalid') ? 404
                    : 400;
